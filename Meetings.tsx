...
const MeetingControls: React.FC<{
    ...
    <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex flex-wrap items-center justify-center gap-3 bg-gray-800/50 dark:bg-gray-900/50 backdrop-blur-md p-3 rounded-full shadow-2xl">
        <button onClick={toggleMic} className={`p-3 rounded-full transition-colors ${isMicOn ? 'bg-gray-600 hover:bg-gray-500' : 'bg-secondary hover:bg-red-500'} text-white`}>
            {isMicOn ? <Mic size={22} /> : <MicOff size={22} />}
        </button>
        <button onClick={toggleCam} className={`p-3 rounded-full transition-colors ${isCamOn ? 'bg-gray-600 hover:bg-gray-500' : 'bg-secondary hover:bg-red-500'} text-white`}>
        ...
        <button onClick={onLeave} className="px-5 py-3 rounded-full bg-secondary hover:bg-red-500 text-white transition-colors"><PhoneOff size={22} /></button>
    </div>
);
        ...
        <div className="flex-1 overflow-y-auto p-2">
            {mockParticipants.map(p => (
                <div key={p.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50">
                    ...
                    <div className="flex items-center gap-3">
                        {p.isMuted ? <MicOff size={18} className="text-gray-400"/> : <Mic size={18} className="text-primary-500"/>}
                        {p.isCameraOn ? <Video size={18} className="text-primary-500"/> : <VideoOff size={18} className="text-gray-400"/>}
                    </div>
                </div>
            ))}
        </div>
...
const ChatPanel: React.FC<{ onClose: () => void }> = ({ onClose }) => (
     <div className="w-full h-full bg-white dark:bg-gray-800 flex flex-col">
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
            <input type="text" placeholder="Send a message..." className="w-full bg-gray-100 dark:bg-gray-700 border-transparent rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary-500"/>
        </div>
    </div>
);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="dark:bg-gray-800 text-center p-8">
                    <div className="mx-auto w-16 h-16 rounded-full bg-primary-100 dark:bg-primary-900/50 text-primary-600 dark:text-primary-300 flex items-center justify-center mb-4">
                        <Plus size={32} />
                    </div>
                    ...
                    {scheduledMeetings.map(meet => (
                        <div key={meet.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                            ...
                            <div className="flex items-center gap-2 mt-2 sm:mt-0">
                                <button className="text-sm text-gray-500 hover:text-primary-600 flex items-center gap-1"><Copy size={14}/> {meet.code}</button>
