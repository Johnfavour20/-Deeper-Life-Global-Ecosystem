...
const NotificationPanel: React.FC<{onClose: () => void}> = ({ onClose }) => {
    ...
    const getIcon = (type: string) => {
        switch(type) {
            case 'sermon': return <Rss className="w-5 h-5 text-primary-500" />;
    ...
    return (
        <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-md shadow-lg z-20 border dark:border-gray-700">
            <div className="p-3 flex justify-between items-center border-b dark:border-gray-700">
                ...
                {unreadCount > 0 && <span className="text-xs bg-primary-500 text-white rounded-full px-2 py-0.5">{unreadCount} New</span>}
            </div>
            <div className="max-h-80 overflow-y-auto">
                {notifications.map(notif => (
                     <div key={notif.id} className={`p-3 flex items-start gap-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 ${!notif.read ? 'bg-primary-50 dark:bg-primary-900/20' : ''}`}>
             ...
             <div className="p-2 border-t dark:border-gray-700 text-center">
                <button className="text-sm font-medium text-primary-600 dark:text-primary-400 w-full hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-md py-1">View All Notifications</button>
            </div>
        </div>
    );
};
                ...
                <div className="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-800 rounded-md shadow-lg py-1 z-10 border dark:border-gray-700">
                    {languages.map(lang => (
                        <a
                            ...
                            className={`block px-4 py-2 text-sm ${streamLanguage === lang ? 'bg-primary-100 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300' : 'text-gray-700 dark:text-gray-200'} hover:bg-gray-100 dark:hover:bg-gray-700`}
                        >
                            {lang}
                        </a>
                    ))}
