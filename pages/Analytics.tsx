    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard icon={Users} title="Total Members" value="50.2M" trend="+1.2M this year" color="blue" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
                    ...
                    <div className="w-full h-64 bg-gray-50 dark:bg-gray-700/50 rounded-lg flex items-end p-4 space-x-2">
                        {analyticsData.membershipGrowth.map((val, i) => (
                            <div key={i} className="flex-1 bg-blue-500 rounded-t-md hover:bg-blue-400 transition-colors" style={{ height: `${(val / Math.max(...analyticsData.membershipGrowth)) * 100}%` }}></div>
                        ))}
                    </div>
                </div>
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
                    ...
                    <div className="w-full h-64 flex items-center justify-center">
                        <div className="relative w-48 h-48 rounded-full" style={{ background: `conic-gradient(from 0deg, #3B82F6 0% ${analyticsData.memberDemographics.data[0]}%, #10B981 ${analyticsData.memberDemographics.data[0]}% ${analyticsData.memberDemographics.data[0] + analyticsData.memberDemographics.data[1]}%, #F97316 ${analyticsData.memberDemographics.data[0] + analyticsData.memberDemographics.data[1]}% 100%)` }}>
                            <div className="absolute inset-4 bg-white dark:bg-gray-800 rounded-full"></div>
                        </div>
                    </div>
                    <div className="flex justify-center space-x-4 mt-4 text-sm">
                        <span className="flex items-center"><div className="w-3 h-3 bg-[#3B82F6] rounded-sm mr-2"></div>Youth</span>
